...
class Whitelist(Arguments.Argument):
    ...
    def __deepcopy__(self, memodict={}):
        return Whitelist(self.list, self.list_from_file, self.in_metadata_path, self.name, self.prefix, self.metadata_rules)

    def _loop(self, command_set):
        logging.log(6, "Looping Whitelist:{}/{}".format(self.list, self.list_from_file))
        ...
        for item in Arguments._generate(command_set):
            ...
            if not m in l:
                logging.log(6, "%s not in whitelist", m)
                continue
            else:
                logging.log(6, "%s in whitelist", m)
            yield item, None
